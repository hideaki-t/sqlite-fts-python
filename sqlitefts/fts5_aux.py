...
from .tokenizer import dll, SQLITE_OK
...
def aux_tokenize(pApi, pFts, pCtx, nVal, apVal):
    ...
    '''
    if nVal != 1:
        dll.sqlite3_error(pCtx, ffi.new(
            'char[]', 'this function accepts only 1 argument'))
        return
    ...
    if rc == SQLITE_OK:
        dll.sqlite3_result_text(pCtx, ffi.new('char []', b', '.join(tokens)),
                                -1, SQLITE_TRANSIENT)
    else:
        dll.sqlite3_error_code(pCtx, rc)
